...
public class PurchaseListActivity2_new extends Activity implements AdapterView.OnItemClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_purchase_list2_new);
		...
		*/
		//신규 등록 버튼
		Button buttonClose = (Button)findViewById(R.id.buttonSendClose);
		...
		//리스트뷰 목록 생성 어댑터
		//String[] from = new String[] {"index", "In_Date", "In_Num", "Office_Code", "Office_Name", "In_Pri", "In_RePri", "Profit_Pri", "Profit_Rate" };
		String[] from = new String[] {"index", "In_Num", "Office_Name", "In_Date", "In_Pri"};
	...
	private void serachPurchaseList(){
		...
		*/
		...
		query = "Select * " +
				"From int_"+tableName+" Where in_date='"+period+"' order by In_Seq ASC ";
		...
		try {
			if ( results.length() > 0 ) {
				...
				for(int i = 0; i < results.length() ; i++) {
					...
					if(son.get("In_Num").toString().contains(posID)){
						...
						map.put("index", i+1+"");
						map.put("In_Num", son.get("In_Num").toString());
						map.put("Office_Name", son.get("Office_Name").toString());
						map.put("In_Date", son.get("In_Date").toString());
						map.put("In_Pri", StringFormat.convertToIntNumberFormat(son.get("In_Pri").toString()));
						mfillMaps.add(map);
					}
				}
			}else{
	...
	*/
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		...
		String in_pri = m_tempInNum.get("In_Pri").replace(",", "");
