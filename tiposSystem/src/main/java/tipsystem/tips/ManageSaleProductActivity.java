...
public class ManageSaleProductActivity extends Activity implements OnItemClickListener,
        OnTabChangeListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_managesale_product);
        ...
        try {
            ...
            /* 김영목 2020.09.14. 바코드 입력 키패드 설정 옵션 추가 */
            m_KeypadType = LocalStorage.getInt(this, "KeypadType:" + m_OFFICE_CODE);
        ...
        String[] from1 = new String[]{"Barcode", "G_Name", "평균판가", "평균원가", "수량", "순매출", "매출원가", "이익금"};
        int[] to1 = new int[]{R.id.item1, R.id.item2, R.id.item3, R.id.item4, R.id.item5, R.id.item6, R.id.item7, R.id.item8};

        adapter1 = new SimpleAdapter(this, mfillMaps1, R.layout.activity_listview_item4_6, from1, to1);
        m_listSalesTab1.setAdapter(adapter1);

        String[] from2 = new String[]{"Barcode", "G_Name", "연도", "일자", "순매출", "순매입", "매출원가합계", "이익금"};
...
*/
    ...
    public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		...
		/*if ( m_listSalesTab1.getId() == arg0.getId() )
		{
	    	...
	    	intent.putExtra("BARCODE", m_barCode.getText().toString());
    ...
    public void OnClickSearch(View v) {
        String m_cuscode = m_customerCode.getText().toString();
        String m_cusname = m_customerName.getText().toString();
        String m_procode = m_barCode.getText().toString();
        String m_proname = m_productName.getText().toString();

        if (m_cuscode.equals("") && m_cusname.equals("") && m_procode.equals("") && m_proname.equals("")) {
            AlertDialog.Builder builder = new AlertDialog.Builder(ManageSaleProductActivity.this,AlertDialog.THEME_HOLO_LIGHT);
            builder.setTitle("알림");
            builder.setMessage("거래처나 상품 중 한가지를 선택해 주세요. \n상품검색 로딩 시간이 길어 집니다. ");
        ...
        for (int y = year1; y <= year2; y++) {
            ...
            for (int m = m1; m <= m2; m++) {
                ...
                query += "Select A.Barcode, B.G_Name, B.Std_Size, '판매수량'=Sum(Case When A.Sale_Yn='1' Then A.Sale_Count Else 0 End), '판매'=Sum(Case When A.Sale_Yn='1' Then A.TSell_Pri+A.Dc_Pri Else 0 End),"
                        ...
                        + " Where " + Card_Yn + " A.Sale_Date >= '" + period1 + "' AND A.Sale_Date <= '" + period2 + "' "
                        + " AND A.office_code LIKE '%" + customerCode + "%' AND A.office_name LIKE '%" + customerName + "%' "
                        + "AND A.barcode LIKE '%" + barCode + "%' AND B.G_Name LIKE '%" + productName + "%' "
	...
	private void queryListForTab1()
	{
		...
		query +=" Select A.Office_Code, A.Office_Name, "
				...
				+" Sum (a.Profit_Pri) '이익금' "
		       	...
		       	+"	Where "+Card_Yn+" A.Office_Code Like '%"+customerCode+"%' And A.Office_Name Like '%"+customerName+"%' " 
	...
	*/
    ...
    private void queryListForTab2() {
        ...
        query = " SELECT A.Barcode, A.G_Name, A.SALE_DATE, A.순매출, A.매출원가합계, ISNULL(B.IN_PRI, 0) '순매입', A.이익금, A.이익률 "
                + " FROM("
                + " Select G.Barcode, G.G_Name, G.SALE_DATE, G.순매출, G.매출원가합계, G.이익금, G.이익률 "
                + " From("
                + "	Select G.Barcode, G.G_Name, G.SALE_DATE, Sum(G.순매출) '순매출', Sum(G.매출원가) '매출원가합계', Sum(G.이익금) '이익금', "
                + "	'이익률'=Case When Sum(G.이익금)=0 Or Sum(G.순매출)=0 Then 0 Else (Sum(G.이익금)/Sum(G.순매출))*100 End "
                + "	From(";
        ...
        for (int y = year1; y <= year2; y++) {
            ...
            for (int m = m1; m <= m2; m++) {
                ...
                query += " Select A.Barcode, A.G_Name, A.SALE_DATE, "
                        + " Sum (a.TSell_Pri - a.TSell_RePri) '순매출', "
                        + " '매출원가'=Sum(Case When A.Sale_YN='1' Then A.Pur_Pri*A.Sale_Count Else A.Pur_Pri*A.Sale_Count End), "
                        + " Sum (a.Profit_Pri) '이익금' "
                        + "	From SaD_" + tableName + " A LEFT JOIN  SaT_" + tableName + " C ON A.Sale_Num=C.Sale_Num "
                        ...
                        + " AND A.Sale_Date >= '" + period1 + "' AND A.Sale_Date <= '" + period2 + "' "
                        + "	AND A.BARCODE LIKE '%" + barCode + "%'  AND A.G_NAME LIKE '%" + productName + "%' "
                        + " Group By A.Barcode, A.G_Name, A.SALE_DATE ";
                query += " UNION ALL ";
            }
        }
        query = query.substring(0, query.length() - 11);
        query += " ) G "
                + " Group By G.Barcode, G.G_Name, G.SALE_DATE "
                + " ) G "
                + " ) A LEFT JOIN ( "
                + " Select Barcode,  IN_DATE, IN_PRI "
                + " From ( "
                + "	SELECT  Barcode,  IN_DATE, SUM(IN_PRI) IN_PRI "
                + " FROM ( ";
        ...
        for (int y = year1; y <= year2; y++) {
            ...
            for (int m = m1; m <= m2; m++) {
                ...
                String tableName = String.format("%04d%02d", y, m);
                query += "	SELECT Barcode, IN_DATE, SUM(IN_PRI) IN_PRI "
                        + " FROM IND_" + tableName + " "
                        + " WHERE IN_DATE BETWEEN '" + period1 + "' AND '" + period2 + "' "
                        + "	AND BARCODE LIKE '%" + barCode + "%' "
                        + " GROUP BY Barcode, IN_DATE ";
        ...
        query = query.substring(0, query.length() - 11);
        query += "	) X "
                + " Group By X.Barcode, X.IN_DATE "
                + " ) X "
                + ") B ON A.Barcode=B.Barcode AND A.SALE_DATE=B.IN_DATE "
                + " ORDER BY A.Barcode, A.G_Name, A.SALE_DATE ";
